...
export const Overview: FC = () => {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl md:text-3xl text-text mb-2">Dashboard Overview</h1>
        ...
        <div className="bg-surface border border-border rounded-xl p-6">
          ...
          {stats.recent_opened.map((r: any, i: number) => (
            <div key={i} className="flex justify-between py-2 border-b border-border last:border-0">
              <span className="text-text/70">{r.name}</span>
              <span className="text-text-muted text-sm">{r.opened_at ? new Date(r.opened_at).toLocaleString() : ''}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 md:mb-6">
        <h1 className="font-display text-2xl md:text-3xl text-text">Guest Management</h1>
        <button onClick={() => setShowAdd(true)} className="btn-primary text-sm">+ Add Guest</button>
      </div>
      ...
      <div className="bg-surface border border-border rounded-xl overflow-x-auto">
        <table className="w-full min-w-[500px]">
          <thead><tr className="border-b border-border">
            ...
            <th className="text-left p-3 md:p-4 text-text-muted text-xs md:text-sm">Status</th>
            <th className="text-left p-3 md:p-4 text-text-muted text-xs md:text-sm">Link</th>
            <th className="text-left p-3 md:p-4 text-text-muted text-xs md:text-sm">Actions</th>
          </tr></thead>
          <tbody>
            {guests.map((g) => (
              <tr key={g.id} className="border-b border-primary/5 hover:bg-primary/5">
                <td className="p-3 md:p-4 font-medium text-text text-sm md:text-base">{g.full_name}</td>
                <td className="p-3 md:p-4 text-text-muted text-xs md:text-sm hidden md:table-cell">{g.phone}</td>
                ...
                <td className="p-3 md:p-4">
                  <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/${coupleSlug}/invite/${g.slug}`); toast.success('Copied!'); }}
                    className="text-primary text-xs hover:underline">Copy Link</button>
                </td>
                <td className="p-3 md:p-4">
      ...
      <div className="flex justify-center gap-2 mt-4">
        {Array.from({ length: Math.ceil(total / 20) }, (_, i) => (
          <button key={i} onClick={() => setPage(i + 1)}
            className={`w-8 h-8 rounded-full text-sm ${page === i + 1 ? 'bg-primary text-surface' : 'bg-surface border border-border'}`}>{i + 1}</button>
        ))}
      </div>
      ...
      {wishes.map((w) => (
        <div key={w.id} className="bg-surface border border-border rounded-xl p-4 mb-3 flex justify-between items-start">
          <div>
            ...
            <p className="text-text-muted text-sm">{w.message}</p>
            <p className="text-text/30 text-xs mt-1">{new Date(w.created_at).toLocaleDateString()}</p>
          </div>
...
export const GalleryManagement: FC = () => {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl md:text-3xl text-text mb-6">Gallery Management</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {photos.map((p) => (
          <div key={p.id} className="relative rounded-xl overflow-hidden group">
            ...
            {p.caption && <p className="text-text-muted text-xs mt-1 text-center">{p.caption}</p>}
            <button onClick={() => handleDelete(p.id)}
              className="absolute top-2 right-2 w-8 h-8 bg-red-500/80 text-surface rounded-full opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-sm">×</button>
          </div>
        ))}
      </div>
    </div>
      ...
      {tracks.map((t) => (
        <div key={t.id} className="bg-surface border border-border rounded-xl p-4 mb-3 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            {!t.is_active && <button onClick={() => handleActivate(t.id)} className="text-primary text-sm">Set Active</button>}
    ...
    <div>
      ...
      {data.data?.map((r: any) => (
        <div key={r.id} className="bg-surface border border-border rounded-xl p-4 mb-3">
          ...
          <p className="text-text-muted text-sm mt-1">{r.attendee_count} guest(s) — {r.message}</p>
        </div>
      ))}
    </div>
  );
